...
fn test_umap_different_dimensions() {
    let data =
        Array2::from_shape_vec((20, 10), (0..200).map(|x| (x as f64) / 10.0).collect()).unwrap();
...
fn test_umap_reproducibility() {
    ...
    let umap1 = UMAP::new().n_neighbors(5).n_epochs(30).random_state(123);

    let umap2 = UMAP::new().n_neighbors(5).n_epochs(30).random_state(123);
...
fn test_umap_builder_pattern() {
    let data = Array2::from_shape_vec((10, 5), vec![1.0; 50]).unwrap();
...
fn test_umap_single_component() {
    let data = Array2::from_shape_vec((15, 10), (0..150).map(|x| x as f64).collect()).unwrap();
...
fn test_umap_larger_dataset() {
    let data =
        Array2::from_shape_vec((50, 20), (0..1000).map(|x| (x as f64).sin()).collect()).unwrap();
